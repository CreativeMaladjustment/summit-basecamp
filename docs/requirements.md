...
| Find your syndicate | Join by invite code, or start a new one | `POST /api/groups`, `POST /api/groups/join` (both wired to the real API — a syndicate created or joined here is real; header/ledger numbers reflect it, but Matchday/Pitch/Bench still show fixed mock fixtures/seats — see Known gaps) |
| Matchday | Next Match hero, Summit Touchline carousel, bench note threads, balance line | `GET /api/groups/{id}/fixtures`, `GET /api/groups/{id}/ledger` (balance line only — see Known gaps), `GET /api/bios/today` |
...
- Frontend and backend are integrated on Landing (sign-in), "Find your syndicate" (create/join, plus skipping straight into a syndicate the signed-in guest slot already belongs to), the Settings profile card (name/phone/email), and now the header's "N seats · Sec/Row" readout and The 14ers ledger card (real package price/seat/member counts and real per-member balances, via `web/src/app.js`'s `paintRealSyndicateDetail`). Matchday, The 14er Pass and The Bench still ship with mock fixtures/seats compiled in at build time (`web/build_src/data.py`), even though each has a live endpoint behind it (`GET .../fixtures`, `GET /api/fixtures/{id}/seats`, `GET .../listings`) and a real, authenticated `groupId` now exists to scope those calls to once they're wired — `src/sync.py`'s weekly roster/fixture sync already creates real `fixtures` rows for a syndicate's upcoming home matches (see **Roster and fixture data sync**), so this is populating a real seat-claim UI from data that may already exist, not building the data source too. Bench note threads have no backend at all yet (no table backs them; `addBenchNoteCard` in `web/src/app.js` is client-only and does not persist). Wiring those up (build-time fetch for public data, runtime session-authenticated fetch for per-member data like balances and seat assignments) is the next major milestone.
