...
7. **Player bios.** One scheduled bio per day (`player_bios.scheduled_date`), surfaced via `GET /api/bios/today`; a cron job (08:00) schedules the next unscheduled bio for today if none is set.
